---
name: awpos-sync-hmac
description: Amankan sinkronisasi offline-online AWPOS dengan HMAC signature dan anti-replay. Gunakan saat implementasi sync push/pull, verifikasi request node, atau R2 object queue. Sesuai doc 08 & 10.
---

# AWPOS — Sync HMAC & Offline Sync

Ikuti `docs/awpos/08_sop_operasional_user_guide.md` dan `docs/awpos/10_template_kode_coding_standard.md`.
...
Header: `X-AWPOS-Node-ID`, `X-AWPOS-Timestamp`, `X-AWPOS-Signature`.
...
5. Duplicate event idempotent (tidak dobel) — lihat `awpos-idempotency`.
...
- File lokal disimpan dulu, masuk `awpos_object_sync_queue`.
