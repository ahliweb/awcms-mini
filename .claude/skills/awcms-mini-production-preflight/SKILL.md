...
→ `modules:compose:check` (ADR-0024 — registry komposisi base valid: DAG,
duplicate module key, capability binding, deployment profile, navigation,
job descriptor) → `test` → `build` → `db:pool:health` (skip bila server
belum jalan, kecuali `APP_ENV=production` — di situ skip BLOKIR go-live) →
