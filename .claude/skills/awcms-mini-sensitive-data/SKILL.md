---
name: awpos-sensitive-data
description: Tangani data sensitif AWPOS (email, phone, WhatsApp, NPWP, NIK, NITKU, receipt token) dengan normalize, hash lookup, dan masking. Gunakan saat menyimpan/menampilkan identifier, membuat profile identifier, atau menyusun DTO/response/log. Sesuai doc 04.
---

# AWPOS — Sensitive Data Handling

Ikuti `docs/awpos/04_erd_data_dictionary.md` (klasifikasi & masking).
...
2. Response umum hanya tampilkan `masked_value`; nilai penuh hanya untuk role berwenang lewat `awpos-abac-guard`.
3. **Jangan** kirim raw value ke response/log/audit/event.
4. Gunakan mapper (`infrastructure/mappers.ts`) untuk mengubah row → safe DTO.
...
- Konsisten dengan redaction logger & `awpos-audit-log`.
