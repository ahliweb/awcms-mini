---
name: awpos-legacy-migration
description: Kerjakan migrasi data legacy ke AWPOS dengan aman (Epic 1). Gunakan saat mengimpor data sistem lama, membuat mapping tabel/field legacy, dry-run backfill, atau menjalankan legacy:preflight. Sesuai checklist legacy doc 07.
---

# AWPOS — Legacy Data Migration

Ikuti `docs/awpos/07_sprint_testing_production_readiness.md` §Legacy migration checklist dan Issue 1.1–1.2 (doc 06 / GitHub #4–#5).
...
2. Import mentah ke schema **`legacy`** terpisah — jangan langsung ke tabel `awpos_*`.
3. Semua run tercatat di `awpos_legacy_migration_runs` (+ mappings, row counts, validation errors, backfill tasks).
4. **Password legacy tidak pernah digunakan ulang** — user diberi reset flow; hash lama tidak diimpor.
5. Identifier (email/phone/NPWP/NIK) dinormalisasi lalu hash+mask via aturan `awpos-sensitive-data`; duplikat di-resolve ke satu profile (bukan buat baru).
...
`awpos-new-migration` (schema toolkit), `awpos-sensitive-data`, `awpos-testing`.
