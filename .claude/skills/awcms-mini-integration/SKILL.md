...
- [ ] **Transactional outbox** — efek samping eksternal (kirim WA/email, upload objek, publish event) ditulis ke tabel outbox **dalam** transaksi bisnis, lalu dispatcher terpisah mengirim di luar transaksi (at-least-once). Bukan panggilan langsung inline. Contoh nyata sudah ada, ikuti pola ini: `awcms_mini_object_sync_queue` + `dispatchObjectSyncQueue` (`src/modules/sync-storage/application/object-dispatch.ts`, Issue #436) — tiga fase CLAIM (transaksi pendek, `FOR UPDATE SKIP LOCKED`, status transien `sending` sebagai lease) → UPLOAD (di luar transaksi apa pun) → FINALIZE (transaksi pendek kedua). `awcms_mini_sync_outbox` (event lokal antar-node) belum punya dispatcher live. Email (epic #492) sedang dibangun mengikuti pola ini: Issue #493 sudah mendefinisikan kontrak `EmailProvider`/DTO dan batas konfigurasi (`src/modules/email/`, provider-neutral, Mailketing adalah satu adapter) — tabel outbox-nya sendiri (`email_messages`, Issue #494) dan dispatcher tiga-fase (Issue #495) belum dibuat. Sampai keduanya landed, `EMAIL_ENABLED=true` hanya lolos validasi config (`bun run config:validate`), belum benar-benar mengirim apa pun.
