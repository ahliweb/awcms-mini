...
preset AND the `news_portal` module enabled for that tenant. A
non-conforming reference is rejected with `422 NEWS_MEDIA_REFERENCE_INVALID`
at post/page create and update, before anything is written. Cross-tenant
references, and references to unverified/failed/orphaned/deleted objects,
are always rejected in this mode. Video gallery items and all behavior
outside full-online R2-only mode are unaffected.
