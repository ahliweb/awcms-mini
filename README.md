...
> **Status:** Sprint foundation Issue 0.1-0.3, seluruh epic Tenant/Identity/Profile (Issue 2.1-2.4, M2), Issue 12.1 (Setup Wizard), dan seluruh epic M5 Sync Storage (Issue 6.1-6.3) tersedia: Astro build, health endpoint, module contract, response helper, soft-delete convention, folder standar, runner PostgreSQL checksum-based, baseline OpenAPI/AsyncAPI dengan validator, skema tenant/office/physical location/tenant settings, skema profile/identifier/channel/address/entity link/merge request, skema identity/tenant user/session dengan endpoint live `POST /auth/login`, `POST /auth/logout`, `GET /auth/me`, skema RBAC/ABAC dengan evaluator default-deny live `POST /access/evaluate`, setup wizard live `GET /setup/status`, `POST /setup/initialize`, sync node/outbox/inbox HMAC-signed live `POST /sync/push`, `POST /sync/pull`, `GET /sync/status`, sync conflict tracking/resolution (bearer session) live `GET /sync/conflicts`, `POST /sync/conflicts/{id}/resolve`, dan R2 object sync queue HMAC-signed live `POST /sync/objects`, `GET /sync/objects/status` — seluruhnya dengan RLS. Modul UI/reporting, deployment masih backlog. Kontributor & coding agent **wajib membaca [`AGENTS.md`](AGENTS.md) lebih dulu**.
...
Backlog base generik ada di [`docs/awcms-mini/06_github_issues_detail.md`](docs/awcms-mini/06_github_issues_detail.md). Foundation **Issue 0.1-0.3**, seluruh epic M2 (**2.1** tenant/office, **2.2** central profile, **2.3** identity/login, **2.4** RBAC/ABAC), **12.1** (setup wizard), dan seluruh epic M5 Sync Storage (**6.1** sync outbox/inbox, **6.2** sync conflict tracking/resolution, **6.3** R2 object sync queue) sudah tersedia; urutan berikutnya: **8.1**/**9.1** (UI/reporting, M7) — hanya bergantung pada M2 yang sudah tuntas. Epic 10/11/12.2 (M8) baru setelah M5+M7 selesai (lihat doc 06 §Koreksi urutan sprint (2)).
