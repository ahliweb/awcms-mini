...
- non-interactive Cloudflare automation should source `CLOUDFLARE_API_TOKEN` from `.env.local` or CI/CD secret storage, not tracked files; Tunnel provisioning needs `Account > Cloudflare Tunnel > Edit`, and DNS provisioning needs zone DNS read/edit permission for the target zone
