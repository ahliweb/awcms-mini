...
> **Status:** Sprint foundation Issue 0.1-0.3 dan seluruh epic Tenant/Identity/Profile (Issue 2.1-2.4, M2) tersedia: Astro build, health endpoint, module contract, response helper, soft-delete convention, folder standar, runner PostgreSQL checksum-based, baseline OpenAPI/AsyncAPI dengan validator, skema tenant/office/physical location/tenant settings, skema profile/identifier/channel/address/entity link/merge request, skema identity/tenant user/session dengan endpoint live `POST /auth/login`, `POST /auth/logout`, `GET /auth/me`, dan skema RBAC/ABAC (permission/role/assignment/decision log) dengan evaluator default-deny live `POST /access/evaluate` — seluruhnya dengan RLS. Modul sync/deployment masih backlog. Kontributor & coding agent **wajib membaca [`AGENTS.md`](AGENTS.md) lebih dulu**.
...
Backlog base generik ada di [`docs/awcms-mini/06_github_issues_detail.md`](docs/awcms-mini/06_github_issues_detail.md). Foundation **Issue 0.1-0.3** dan seluruh epic M2 (**2.1** tenant/office, **2.2** central profile, **2.3** identity/login, **2.4** RBAC/ABAC) sudah tersedia; urutan berikutnya: **12.1** (setup wizard).
...
**Semantic Versioning** + **[Changesets](.changeset/README.md)**; riwayat di [`CHANGELOG.md`](CHANGELOG.md). Setiap PR yang mengubah perilaku wajib menyertakan changeset. Versi saat ini `0.5.0` (Foundation skeleton SSR + Issue 2.1-2.4: tenant/office, central profile, identity/login, RBAC/ABAC).
