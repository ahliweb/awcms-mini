...
- ABAC: default-deny, permission key baru per resource (`document_infrastructure.documents.*`, `.classifications.*`, `.versions.*`, `.relations.*`, `.sequences.*`, `.reservations.*`, `.evidence.*`) — lihat migration permission seed. Akses dokumen menggabungkan tenant + classification/confidentiality + permission eksplisit (issue #751 "Security and integrity requirements").
