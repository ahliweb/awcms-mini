...
| [0017](0017-document-infrastructure-module-admission.md)        | Admission modul `document_infrastructure` — generic document registry, versioning, classification, evidence, dan numbering sequences, dependency `tenant_admin`+`identity_access`+`domain_event_runtime`                                                               | Accepted |
| [0018](0018-integration-hub-module-admission.md)                | Admission modul `integration_hub` (System Foundation) — signed inbound webhook, outbound event subscription, replay protection, adapter health, dependency `tenant_admin`+`identity_access`+`domain_event_runtime`, provider `IntegrationAdapterPort`                  | Accepted |
