...
| [0017](0017-integration-hub-module-admission.md)                | Admission modul `integration_hub` (System Foundation) — signed inbound webhook, outbound event subscription, replay protection, adapter health, dependency `tenant_admin`+`identity_access`+`domain_event_runtime`, provider `IntegrationAdapterPort`                  | Accepted |
