...
This runbook covers urgent recovery procedures for the Mini governance and security stack.
...
Use the admin surfaces added in `awcms-users-admin`:

- User detail tabs for `Overview`, `Roles`, `Sessions`, and `Security`
- `Security Settings` for mandatory 2FA rollout mode
- `Audit Logs` for verification after recovery
...
Expected system effects of the forced reset flow:
...
Use this when the incident is caused by a recent rollout, especially around authorization or mandatory 2FA enforcement.
...
Prefer reducing scope over fully disabling controls unless recovery speed requires it.
...
4. Verify that the request is now allowed with reason `ALLOW_ABAC_AUDIT_ONLY` and that the audit-only reporter path is capturing the event.
5. Open a follow-up issue to remove the temporary audit-only mode once the policy or data problem is corrected.

Do not remove permission checks or authorization rules from production code as an emergency shortcut.
...
- Direct SQL updates to passwords, TOTP credentials, recovery codes, or role protection state unless a separately approved incident procedure exists
...
- Can a protected-role user be recovered with only documented admin actions?
- Can lockout recovery be explained without relying on runtime internals?
