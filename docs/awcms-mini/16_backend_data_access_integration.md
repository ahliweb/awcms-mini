...
- Issue #599 menggeneralisasi pengecualian "bukan infra failure" di atas: `withTenant` skip `circuitBreaker.recordFailure()` untuk **semua** `Bun.SQL.PostgresError` SQLSTATE kelas `23` (integrity constraint violation — FK/unique/check violation), bukan cuma `IdempotencyRaceLostError`. Sebelumnya, `INSERT`/`UPDATE` apa pun yang gagal karena FK/unique constraint (mis. `tenantId` caller-supplied yang tak ada di `GET /api/v1/auth/providers/google/start`, ditemukan lewat security review PR #598) ikut menghitung sebagai kegagalan infra dan bisa membuka circuit breaker aplikasi-lebar dari beberapa request ber-input invalid saja — lihat doc `database-pooling.md` §4 dan `.changeset/database-circuit-breaker-integrity-violations.md`. Issue #601 memperluas pengecualian yang sama ke SQLSTATE kelas `22` (data exception, mis. `22P02` string bukan-UUID yang dibandingkan ke kolom `uuid`) — kelas bug struktural yang sama persis, ditutup sebelum ada endpoint yang mereproduksinya.
