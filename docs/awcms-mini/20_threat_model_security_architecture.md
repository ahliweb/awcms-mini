...
| **Idempotency-Key yang SAMA dipakai ulang lintas DUA resource berbeda menyebabkan false replay** (Issue #795, recurring class ditemukan pertama kali di PR #783/#750 reference-data — DIPERBAIKI, bukan diabaikan)                                                                                   | `request_scope` pada penyimpanan idempotency bersifat per-TIPE-endpoint, dibagi oleh SELURUH resource tipe itu dalam satu tenant — bukan per-resource. `documents/{id}/restore` dan `classifications/{id}/restore` sebelumnya meng-hash body kosong (`computeRequestHash({})`, TIDAK menyertakan `{id}`); `documents/{id}` DELETE, `classifications/{id}` DELETE, `documents/{id}/relations/{relationId}` DELETE, `reservations/{id}/cancel`, dan `reservations/{id}/commit` meng-hash body tanpa menyertakan path param yang menunjuk resource yang dimutasi. Reuse key lintas dua resource berbadan sama akan mereplay respons resource PERTAMA untuk request yang seharusnya memutasi resource KEDUA (double-submit protection salah sasaran, bukan sekadar tidak berfungsi). Diperbaiki: hash kini menyertakan path param identitas resource (`id`/`relationId`) DAN literal `action` eksplisit (`computeRequestHash({ id, action: "restore" })` untuk pure-action-trigger, `computeRequestHash({ ...body, id, action: "delete" })` dst. untuk endpoint dengan body nyata). `sequences/revise`/`restore`/`deactivate` DIPERIKSA dan TIDAK rentan — ketiganya endpoint index-level (bukan `[id]`/`[key]` path-scoped); identitas resource (`scopeType`+`scopeId`+`sequenceKey`) sudah bagian dari body mentah yang di-hash. Diuji adversarial (reused key lintas 2 resource berbeda -> `409 IDEMPOTENCY_CONFLICT`, state DB resource kedua terbukti TIDAK berubah, lalu berhasil dengan key sendiri) untuk document restore, classification restore, dan reservation cancel di `tests/integration/document-infrastructure.integration.test.ts`.                                                                                                                                                                                                                                                                                                                                                                          |
