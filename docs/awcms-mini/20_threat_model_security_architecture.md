...
## Integration Hub (Issue #754, epic `platform-evolution` #738 Wave 3, ADR-0018)
...
- **Gating confidentiality-tier hanya pada jalur baca (`documents`/`versions`/`relations` GET), belum pada mutasi atau `evidence`/`reservations`** — permission action-spesifik (mis. `documents.void`, `versions.create`) tetap satu-satunya gate untuk mutasi terhadap dokumen `confidential`/`restricted`, sengaja dipisah dari dimensi "siapa boleh membaca level apa" (dua sumbu otorisasi berbeda). `GET .../evidence`/`GET .../reservations` juga belum memfilter berdasarkan confidentiality dokumen induknya. Tidak ada audit-log/decision-log entry terpisah untuk keputusan tier baca ini sendiri (mengikuti preseden `raw_detail.read`, yang juga tidak mencatat decision-log untuk tier check-nya).
