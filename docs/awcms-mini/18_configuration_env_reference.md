...
```mermaid
flowchart LR
  Def[Default kode] --> Env[Environment variable] --> Set[awcms_tenant_settings - per tenant] --> Eff[Konfigurasi efektif]
```

- Runtime/secret (DB, JWT, HMAC, provider key): dari **environment**.
- Preferensi tenant (locale, theme, flag fitur tampilan): dari **`awcms_tenant_settings`**.
...
# Database
DATABASE_URL=postgres://awcms-mini:awcms_password@localhost:5432/awcms-mini
...
- Preferensi tenant (locale/theme) dari `awcms_tenant_settings`, bukan hardcode.
