...
2. **Keyboard-first untuk kasir** — semua aksi POS dapat tanpa mouse.
...
7. **Responsif** — admin desktop-first, kasir fullscreen, customer portal mobile-first.
...
Aturan: default `system`; pilihan user disimpan (localStorage + `awcms_tenant_settings.default_theme`); `data-theme` di-set pada `<html>` sebelum paint untuk mencegah flash.
...
- Locale awal: **id**, **en** (siap ms/ar). Default dari `awcms_tenants.default_locale`.
- Format PO/message catalog (migration `009_awcms_i18n_po_schema.sql`).
