...
Blok di atas juga diperluas secara ADITIF oleh sejumlah field opsional — `capabilities?: ModuleCapabilityContract` (ADR-0011), `compatibility.deploymentProfiles?` (ADR-0014), `dataLifecycle?` (#745), `sodRules?` (#746), `dataExchange?` (#752), `referenceData?` (#750), `reportingProjections?` (#753), `defaultTenantState?` + `serviceCatalog?` (#870/#874), dan `serviceLevelObjectives?` (#930). Lihat `src/modules/_shared/module-contract.ts` untuk bentuk lengkap terkini (kode tetap sumber kebenaran; blok di atas sengaja tidak disalin ulang seluruhnya di sini supaya tidak drift).

File itu juga mengekspor `MODULE_CONTRACT_VERSION` — SemVer bentuk kontrak `ModuleDescriptor` itu sendiri, independen dari `package.json` dan dari kontrak OpenAPI/AsyncAPI (ADR-0008). Bump MAJOR hanya bila field dihapus/rename/opsional-jadi-wajib; MINOR untuk field opsional baru; PATCH dokumentasi saja. Nilai saat ini ada di kode — **jangan menyalin angkanya ke dokumen ini**, karena versi yang di-hardcode di prosa adalah persis yang paling sering basi (dokumen ini sempat menyebut `"1.0.0"` lama setelah kontraknya sampai `2.x`).

> **Catatan ADR-0024.** Tipe `ApplicationModuleRegistry`/`ModuleMigrationNamespace`, gerbang `bun run extension:check`, dan `EXTENSION_MANIFEST_SCHEMA_VERSION` yang dulu disebut di paragraf ini **sudah dihapus** — keluarga AWCMS kini template dipakai-langsung tanpa repo turunan. Validasi komposisi registry sekarang lewat `bun run modules:compose:check`.
