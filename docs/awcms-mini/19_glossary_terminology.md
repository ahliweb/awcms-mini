...
| Istilah                                    | Definisi                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Tenant**                                 | Unit isolasi data & langganan platform (`awcms_mini_tenants`) — **batas keamanan (RLS)**, satu tenant = satu dataset terisolasi. Tidak pernah dilemahkan oleh legal entity/organization unit.                                                                                                                                                                                                       |
...
| **Organization unit**                      | Subdivisi bisnis (departemen/cabang/cost center) di dalam legal entity/tenant — batas bisnis/akuntansi/workflow, berbeda dari `awcms_mini_offices` (register lokasi fisik yang sudah ada).                                                                                                                                                                                                          |
...
| **SaaS Control Plane**                     | Lapisan control-plane pemilik service catalog & subscription billing lintas tenant. Per **ADR-0022** tujuh modulnya (`service_catalog`, `tenant_entitlement`, `tenant_provisioning`, `tenant_lifecycle`, `usage_metering`, `subscription_billing`, `payment_gateway`) diadmisi **in-repo** sebagai Official Optional Business Foundation (default-disabled) — bukan repo terpisah — lihat ADR-0022. |
