...
Modul domain (mis. `catalog-inventory`, `sales-pos`, `warehouse-management`, `accounting-tax`, `crm-communication`, `ai-analyst` pada contoh AWPOS) ditambahkan **langsung** di `src/modules/` template ini/fork yang dipakai (ADR-0024), divalidasi `bun run modules:compose:check` — bukan di repo aplikasi terpisah.
...
**Diperbarui oleh ADR-0024.** Jalur aplikasi-turunan di repo terpisah — beserta seluruh mekanismenya (seam `src/modules/application-registry.ts`, `composeModuleRegistry()` untuk registry turunan, namespace migration turunan `900–999`/`ApplicationModuleRegistry.migrationNamespace`, dan gerbang `extension:check`) — telah **DIHAPUS** (ADR-0024 men-supersede ADR-0013/0014/0015). Model sekarang: modul domain ditambahkan **langsung** di `src/modules/` template ini/fork yang dipakai, dan migration domainnya melanjutkan nomor berurutan setelah migration terakhir base saat ini — tidak ada namespace migration terpisah. Komposisi registry base divalidasi otomatis oleh `bun run modules:compose:check` (dan `modules:composition:inventory:check`).
