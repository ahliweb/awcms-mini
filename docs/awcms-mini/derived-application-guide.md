...
> **⚠️ DEPRECATED ([ADR-0024](../adr/0024-awcms-family-direct-use-templates-and-derived-pathway-removal.md)).** Model aplikasi-turunan di repo terpisah DICABUT — keluarga AWCMS (`awcms-mini`/`awcms`/`awcms-micro`) kini template **dipakai-langsung**, tanpa membuat repo derivatif (kembangkan modul domain langsung di template). Seam `src/modules/application-registry.ts`, gerbang `bun run extension:check`, mesin manifest kompatibilitas, dan konsep migration namespace 900–999 sudah **dihapus** dari repo ini. Dokumen ini dipertahankan sebagai catatan historis.
