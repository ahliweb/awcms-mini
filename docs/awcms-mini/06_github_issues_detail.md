...
**Acceptance criteria:** Struktur tersedia, build pass, health endpoint ada, README menjelaskan stack, tidak ada secret.
...
**Acceptance criteria:** API spec valid, AsyncAPI valid, shared response schema tersedia, HMAC sync header terdokumentasi.
...
**Scope:** `awcms_legacy_migration_runs`, mappings, row counts, validation errors, backfill tasks, mapping awal tabel legacy.
...
**Scope:** `awcms_tenants`, `awcms_offices`, `awcms_tenant_settings`, `awcms_physical_locations`, RLS, unique tenant/office code.

**Acceptance criteria:** Tenant dan office dapat dibuat, tipe office lengkap, duplicate ditolak, tenant inactive ditolak transaksi.

## Issue 2.2 — Add Central Profile Schema

**Scope:** `awcms_profiles`, identifiers, channels, addresses, entity links, audit logs, merge request.

**Acceptance criteria:** Profile bisa link ke user/customer/tax/CRM; identifier dimasking; duplicate resolver bekerja.
...
**Acceptance criteria:** Default deny, deny overrides allow, kasir ditolak akses tax/admin, decision log tercatat.
...
**Scope:** Product, category, brand, unit, product unit, price, endpoint CRUD/search.

**Acceptance criteria:** SKU unik, barcode unik, harga aktif, product inactive tidak bisa dijual.
...
**Acceptance criteria:** Cart dibuat, item tambah/update/delete, total server-side.
...
**Scope:** Warehouse, zone, bin, bin balance, warehouse APIs.

**Acceptance criteria:** Warehouse dari office, bin unique per warehouse, stok per bin terlihat.
...
**Scope:** Receipt template, PDF generation, local file metadata, `awcms_receipt_pdfs`.
