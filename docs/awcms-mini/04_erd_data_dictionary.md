...
Belum ada middleware collector (Issue #620), UA parser (#619), API/dashboard (#621/#622), enrichment geolokasi (#623), atau job rollup/retensi (#624) — tabel-tabel di atas belum menerima tulisan apa pun sampai issue-issue tersebut.
...
| — `awcms_mini_visit_events`/raw detail (Issue #617, epic: visitor analytics) |                                                                                                                                                                                                                                                 Event: default **90 hari** (`VISITOR_ANALYTICS_EVENT_RETENTION_DAYS`); raw IP/user-agent detail: default **30 hari** (`VISITOR_ANALYTICS_RAW_DETAIL_RETENTION_DAYS`), lebih pendek karena kelas data lebih sensitif (doc 18 §Visitor analytics). Purge job belum ada — fast-follow Issue #624, akan meniru pola batch-delete `logs:audit:purge`. |
| — `awcms_mini_visitor_daily_rollups` (Issue #617, epic: visitor analytics)   |                                                                                                                                                                                                                                                                                                                                                                                                                       Default **730 hari** (`VISITOR_ANALYTICS_ROLLUP_RETENTION_DAYS`) — agregat, bukan raw data, jadi retensi jauh lebih panjang. Purge job belum ada — fast-follow Issue #624. |
