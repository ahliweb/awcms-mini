...
- Schema `BlogPageItem`.`contentJson`: When full-online R2-only news portal mode is active for this tenant (Issue #636), an image gallery block item (mediaType image) must use mediaObjectId referencing a verified/attached same-tenant media object, never a raw url. A video_news block (Issue #639) requires an allowlisted provider (currently only youtube) and a valid videoId (normalized server-side from a bare id or common video URL); its optional thumbnailMediaObjectId, when full-online R2-only mode is active, must likewise reference a verified/attached same-tenant media object.
...
- Schema `CreateBlogPageRequest`.`contentJson`: When full-online R2-only news portal mode is active for this tenant (Issue #636), an image gallery block item (mediaType image) must use mediaObjectId referencing a verified/attached same-tenant media object, never a raw url. A video_news block (Issue #639) requires an allowlisted provider (currently only youtube) and a valid videoId (normalized server-side from a bare id or common video URL); its optional thumbnailMediaObjectId, when full-online R2-only mode is active, must likewise reference a verified/attached same-tenant media object.
- Schema `CreateBlogPageRequest`.`featuredMediaId`: When full-online R2-only news portal mode is active for this tenant (Issue #636), must reference an existing, same-tenant media object (Issue #633) with status verified or attached — never a local path or arbitrary external URL. A non-conforming reference is rejected with 422 NEWS_MEDIA_REFERENCE_INVALID. Outside that mode, only the UUID shape is validated.
- Schema `CreateBlogPostRequest`.`contentJson`: When full-online R2-only news portal mode is active for this tenant (Issue #636), an image gallery block item (mediaType image) must use mediaObjectId referencing a verified/attached same-tenant media object, never a raw url. A video_news block (Issue #639) requires an allowlisted provider (currently only youtube) and a valid videoId (normalized server-side from a bare id or common video URL); its optional thumbnailMediaObjectId, when full-online R2-only mode is active, must likewise reference a verified/attached same-tenant media object.
- Schema `CreateBlogPostRequest`.`featuredMediaId`: When full-online R2-only news portal mode is active for this tenant (Issue #636), must reference an existing, same-tenant media object (Issue #633) with status verified or attached — never a local path or arbitrary external URL. A non-conforming reference is rejected with 422 NEWS_MEDIA_REFERENCE_INVALID. Outside that mode, only the UUID shape is validated.
- Schema `UpdateBlogPageRequest`.`contentJson`: When full-online R2-only news portal mode is active for this tenant (Issue #636), an image gallery block item (mediaType image) must use mediaObjectId referencing a verified/attached same-tenant media object, never a raw url. A video_news block (Issue #639) requires an allowlisted provider (currently only youtube) and a valid videoId (normalized server-side from a bare id or common video URL); its optional thumbnailMediaObjectId, when full-online R2-only mode is active, must likewise reference a verified/attached same-tenant media object.
- Schema `UpdateBlogPageRequest`.`featuredMediaId`: When full-online R2-only news portal mode is active for this tenant (Issue #636), must reference an existing, same-tenant media object (Issue #633) with status verified or attached — never a local path or arbitrary external URL. A non-conforming reference is rejected with 422 NEWS_MEDIA_REFERENCE_INVALID. Outside that mode, only the UUID shape is validated.
- Schema `UpdateBlogPostRequest`.`contentJson`: When full-online R2-only news portal mode is active for this tenant (Issue #636), an image gallery block item (mediaType image) must use mediaObjectId referencing a verified/attached same-tenant media object, never a raw url. A video_news block (Issue #639) requires an allowlisted provider (currently only youtube) and a valid videoId (normalized server-side from a bare id or common video URL); its optional thumbnailMediaObjectId, when full-online R2-only mode is active, must likewise reference a verified/attached same-tenant media object.
...
| 413    | Request body exceeds the endpoint's size limit (Issue #686, epic #679) — either its declared `Content-Length` or, for a chunked/ unlabeled body, the actual streamed byte count. Error code `PAYLOAD_TOO_LARGE`.                                                                                                                                                         | [`ApiError`](#standard-error-envelope)                                                   |
| 422    | NEWS_MEDIA_REFERENCE_INVALID (Issue #636; extended by Issue #639 for a video_news block's thumbnailMediaObjectId) — full-online R2-only news portal mode is active for this tenant and featuredMediaId, a contentJson image gallery item, or a video_news block's thumbnailMediaObjectId does not reference an existing, same-tenant, verified/attached R2 media object. | [`ApiError`](#standard-error-envelope)                                                   |
| 500    | Internal server error without stack trace.                                                                                                                                                                                                                                                                                                                               | [`ApiError`](#standard-error-envelope)                                                   |
...
| 409    | A post already exists for this slug in this locale.                                                                                                                                                                                                                                                                                                                              | [`ApiError`](#standard-error-envelope)                                                   |
| 413    | Request body exceeds the endpoint's size limit (Issue #686, epic #679) — either its declared `Content-Length` or, for a chunked/ unlabeled body, the actual streamed byte count. Error code `PAYLOAD_TOO_LARGE`.                                                                                                                                                                 | [`ApiError`](#standard-error-envelope)                                                   |
| 422    | NEWS_MEDIA_REFERENCE_INVALID (Issue #636; extended by Issue #639 for a video_news block's thumbnailMediaObjectId) — full-online R2-only news portal mode is active for this tenant and a submitted featuredMediaId, contentJson image gallery item, or video_news block's thumbnailMediaObjectId does not reference an existing, same-tenant, verified/attached R2 media object. | [`ApiError`](#standard-error-envelope)                                                   |
| 500    | Internal server error without stack trace.                                                                                                                                                                                                                                                                                                                                       | [`ApiError`](#standard-error-envelope)                                                   |
...
| 404    | Post not found, or revision not found for this post.                                                                                                                                                                                                                                                                                                                                                                                                                            | [`ApiError`](#standard-error-envelope)                                                   |
| 409    | Idempotency-Key reused with a different request.                                                                                                                                                                                                                                                                                                                                                                                                                                | [`ApiError`](#standard-error-envelope)                                                   |
| 422    | NEWS_MEDIA_REFERENCE_INVALID (Issue #636; extended by Issue #639 for video_news thumbnails, and for an invalid video_news block provider/videoId) — full-online R2-only news portal mode is active for this tenant and the revision's contentJson references image(s)/video thumbnail(s) that are not valid R2 media objects (e.g. a raw URL from before the mode was activated), or contains an invalid video_news block — the revision cannot be restored onto the live post. | [`ApiError`](#standard-error-envelope)                                                   |
| 500    | Internal server error without stack trace.                                                                                                                                                                                                                                                                                                                                                                                                                                      | [`ApiError`](#standard-error-envelope)                                                   |
...
### `GET /api/v1/blog/internal-tag-links/settings` — Read this tenant's automatic internal tag linking settings

- **operationId**: `blogInternalTagLinksSettingsGet`
- **Security**: bearerAuth + tenantHeader
...
| Status | Description                                                                                                               | Schema                                                                                                                       |
| ------ | ------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| 200    | Effective automatic internal tag linking configuration (deployment-wide knobs plus this tenant's own overridable policy). | [`ApiSuccess`](#standard-success-envelope)&lt;[`InternalTagLinkingSettingsView`](#schema-internaltaglinkingsettingsview)&gt; |
| 400    | Validation or request error.                                                                                              | [`ApiError`](#standard-error-envelope)                                                                                       |
| 401    | Authentication required or expired.                                                                                       | [`ApiError`](#standard-error-envelope)                                                                                       |
| 403    | Access denied by RBAC, ABAC, or tenant policy.                                                                            | [`ApiError`](#standard-error-envelope)                                                                                       |
| 500    | Internal server error without stack trace.                                                                                | [`ApiError`](#standard-error-envelope)                                                                                       |

### `PATCH /api/v1/blog/internal-tag-links/settings` — Update this tenant's automatic internal tag linking settings

- **operationId**: `blogInternalTagLinksSettingsUpdate`
- **Security**: bearerAuth + tenantHeader
...
**Request body** (required): [`UpdateInternalTagLinkingSettingsInput`](#schema-updateinternaltaglinkingsettingsinput)

**Responses**

| Status | Description                                                                                                                                                                                                      | Schema                                                                                                                       |
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| 200    | Settings updated (partial update — only fields present in the request body are changed).                                                                                                                         | [`ApiSuccess`](#standard-success-envelope)&lt;[`InternalTagLinkingSettingsView`](#schema-internaltaglinkingsettingsview)&gt; |
| 400    | Validation or request error.                                                                                                                                                                                     | [`ApiError`](#standard-error-envelope)                                                                                       |
| 401    | Authentication required or expired.                                                                                                                                                                              | [`ApiError`](#standard-error-envelope)                                                                                       |
| 403    | Access denied by RBAC, ABAC, or tenant policy.                                                                                                                                                                   | [`ApiError`](#standard-error-envelope)                                                                                       |
| 413    | Request body exceeds the endpoint's size limit (Issue #686, epic #679) — either its declared `Content-Length` or, for a chunked/ unlabeled body, the actual streamed byte count. Error code `PAYLOAD_TOO_LARGE`. | [`ApiError`](#standard-error-envelope)                                                                                       |
...
### `GET /api/v1/blog/posts/{id}/internal-links/preview` — Preview which terms would be automatically linked in a post's rendered content

- **operationId**: `blogPostInternalLinksPreview`
- **Security**: bearerAuth + tenantHeader
...
| Status | Description                                                                             | Schema                                                                                                                         |
| ------ | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| 200    | Which tag terms would be auto-linked in this post's current content, before publishing. | [`ApiSuccess`](#standard-success-envelope)&lt;[`InternalTagLinkingPreviewResult`](#schema-internaltaglinkingpreviewresult)&gt; |
| 400    | Validation or request error.                                                            | [`ApiError`](#standard-error-envelope)                                                                                         |
| 401    | Authentication required or expired.                                                     | [`ApiError`](#standard-error-envelope)                                                                                         |
| 403    | Access denied by RBAC, ABAC, or tenant policy.                                          | [`ApiError`](#standard-error-envelope)                                                                                         |
| 404    | Resource not found or hidden by soft-delete policy.                                     | [`ApiError`](#standard-error-envelope)                                                                                         |
...
| 409    | Idempotency-Key reused with a different request.                                                                                                                                                                 | [`ApiError`](#standard-error-envelope)                                                             |
| 413    | Request body exceeds the endpoint's size limit (Issue #686, epic #679) — either its declared `Content-Length` or, for a chunked/ unlabeled body, the actual streamed byte count. Error code `PAYLOAD_TOO_LARGE`. | [`ApiError`](#standard-error-envelope)                                                             |
| 422    | The registered provider adapter for `providerKey` (Issue                                                                                                                                                         | [`ApiError`](#standard-error-envelope)                                                             |
| 500    | Internal server error without stack trace.                                                                                                                                                                       | [`ApiError`](#standard-error-envelope)                                                             |
...
### `POST /api/v1/social-publishing/accounts/{id}/verify` — Verify a connected account can be posted to

- **operationId**: `socialPublishingAccountsVerify`
- **Security**: bearerAuth + tenantHeader

Generic, provider-neutral "verify connection" action (Issue #646) — calls the registered provider adapter's `verifyCredentials` (e.g. the Telegram channel adapter checks the bot is a channel administrator with post permission; the Meta adapter, Issue #644, checks the access token via `debug_token` and that it can reach the connected Page/Instagram Business account). Requires `Idempotency-Key` (real outbound provider call, same external-call class as `accounts.connect`/`.disconnect`), gated by `accounts.verify` (not classified high-risk — only updates `lastVerifiedAt`/`scopes`, never `tokenReference`). A failed check is still a `200` with `valid: false` — informational, never itself a state transition on the account.
...
| Status | Description                                                                                                                                                                                                      | Schema                                                                                                             |
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| 200    | Verification check result.                                                                                                                                                                                       | [`ApiSuccess`](#standard-success-envelope)&lt;[`SocialAccountVerifyResult`](#schema-socialaccountverifyresult)&gt; |
| 400    | Validation or request error.                                                                                                                                                                                     | [`ApiError`](#standard-error-envelope)                                                                             |
| 401    | Authentication required or expired.                                                                                                                                                                              | [`ApiError`](#standard-error-envelope)                                                                             |
| 403    | Access denied by RBAC, ABAC, or tenant policy.                                                                                                                                                                   | [`ApiError`](#standard-error-envelope)                                                                             |
| 404    | Resource not found or hidden by soft-delete policy.                                                                                                                                                              | [`ApiError`](#standard-error-envelope)                                                                             |
| 409    | Idempotency-Key reused with a different request, or the account is disconnected.                                                                                                                                 | [`ApiError`](#standard-error-envelope)                                                                             |
| 413    | Request body exceeds the endpoint's size limit (Issue #686, epic #679) — either its declared `Content-Length` or, for a chunked/ unlabeled body, the actual streamed byte count. Error code `PAYLOAD_TOO_LARGE`. | [`ApiError`](#standard-error-envelope)                                                                             |
| 500    | Internal server error without stack trace.                                                                                                                                                                       | [`ApiError`](#standard-error-envelope)                                                                             |
...
| `contentJson`        | object                                                        | yes      | no       | When full-online R2-only news portal mode is active for this tenant (Issue #636), an image gallery block item (mediaType image) must use mediaObjectId referencing a verified/attached same-tenant media object, never a raw url. A video_news block (Issue #639) requires an allowlisted provider (currently only youtube) and a valid videoId (normalized server-side from a bare id or common video URL); its optional thumbnailMediaObjectId, when full-online R2-only mode is active, must likewise reference a verified/attached same-tenant media object. |
...
| `visibility`                   | enum(`public`, `private`, `unlisted`)                                    | yes      | no       |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `featuredMediaId`              | string (uuid)                                                            | yes      | yes      | When full-online R2-only news portal mode is active for this tenant (Issue #636), must reference an existing, same-tenant media object (Issue #633) with status verified or attached — never a local path or arbitrary external URL. A non-conforming reference is rejected with 422 NEWS_MEDIA_REFERENCE_INVALID. Outside that mode, only the UUID shape is validated.                                                                                                                                                                                                                                        |
| `seoImageMediaId`              | string (uuid)                                                            | yes      | yes      | Issue #649 — explicit social/SEO preview image override, same validation as featuredMediaId. Takes priority over featuredMediaId when resolving og:image/twitter:image/JSON-LD image for public sharing.                                                                                                                                                                                                                                                                                                                                                                                                       |
| `seoTitle`                     | string                                                                   | yes      | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `metaDescription`              | string                                                                   | yes      | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `canonicalUrl`                 | string                                                                   | yes      | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `locale`                       | string                                                                   | yes      | no       |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `restoredBy`                   | string (uuid)                                                            | yes      | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `version`                      | integer                                                                  | yes      | no       |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `autoInternalTagLinksDisabled` | boolean                                                                  | yes      | no       | Manual per-post opt-out of automatic internal tag linking (Issue #641). Defaults false.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `socialPreviewFallbackImageMediaId`        | string (uuid)                                                            | yes      | yes      | Issue #649 — tenant-level R2 fallback social preview image (lowest-priority source), re-verified at render time via the same R2-only mechanism as featuredMediaId. null when never configured. |
| `socialPreviewContentImageFallbackEnabled` | boolean                                                                  | yes      | no       | Issue #649 — whether the first verified R2 image found in a post's own content may be used as its social preview when no explicit SEO image or featured image is set. Defaults to true.        |
...
```json
{
  ...
  "socialPreviewFallbackImageMediaId": "00000000-0000-0000-0000-000000000000",
  "socialPreviewContentImageFallbackEnabled": false,
  "updatedAt": "2026-01-01T00:00:00.000Z"
}
```
...
| `visibility`                   | enum(`public`, `private`, `unlisted`) | no       | no       |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `featuredMediaId`              | string (uuid)                         | no       | yes      | When full-online R2-only news portal mode is active for this tenant (Issue #636), must reference an existing, same-tenant media object (Issue #633) with status verified or attached — never a local path or arbitrary external URL. A non-conforming reference is rejected with 422 NEWS_MEDIA_REFERENCE_INVALID. Outside that mode, only the UUID shape is validated.                                                                                                                                                                                          |
| `seoImageMediaId`              | string (uuid)                         | no       | yes      | Issue #649 — explicit social/SEO preview image override, same validation as featuredMediaId. Takes priority over featuredMediaId when resolving og:image/twitter:image/JSON-LD image for public sharing.                                                                                                                                                                                                                                                                                                                                                         |
| `seoTitle`                     | string                                | no       | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `metaDescription`              | string                                | no       | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `canonicalUrl`                 | string                                | no       | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `termIds`                      | array of string (uuid)                | no       | no       | Category/tag ids to assign to this post (Issue #539).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `autoInternalTagLinksDisabled` | boolean                               | no       | no       | Manual per-post opt-out of automatic internal tag linking (Issue #641). Defaults false.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Field            | Type                                                                    | Required | Nullable | Description |
| ---------------- | ----------------------------------------------------------------------- | -------- | -------- | ----------- |
| `postId`         | string (uuid)                                                           | yes      | no       |             |
| `enabled`        | boolean                                                                 | yes      | no       |             |
| `disabledReason` | enum(`deployment_disabled`, `tenant_disabled`, `post_disabled`, `null`) | yes      | yes      |             |
| `matches`        | array of [`InternalTagLinkMatch`](#schema-internaltaglinkmatch)         | yes      | no       |             |
| `totalLinked`    | integer                                                                 | yes      | no       |             |
| `maxPerPost`     | integer                                                                 | yes      | no       |             |
| `maxPerTag`      | integer                                                                 | yes      | no       |             |
...
### Schema: InternalTagLinkingSettingsView

Effective automatic internal tag linking configuration (Issue #641).

| Field                     | Type                   | Required | Nullable | Description                                                                                             |
| ------------------------- | ---------------------- | -------- | -------- | ------------------------------------------------------------------------------------------------------- |
| `enabled`                 | boolean                | yes      | no       | Final resolved enabled flag (deploymentEnabled AND tenantEnabled).                                      |
| `deploymentEnabled`       | boolean                | yes      | no       | Deployment-wide BLOG_AUTO_INTERNAL_TAG_LINKS_ENABLED kill switch — read-only from this endpoint.        |
| `tenantEnabled`           | boolean                | yes      | no       | This tenant's own enable/disable override.                                                              |
| `caseInsensitive`         | boolean                | yes      | no       |                                                                                                         |
| `disabledTagIds`          | array of string (uuid) | yes      | no       |                                                                                                         |
| `maxPerPost`              | integer                | yes      | no       | Deployment-wide BLOG_AUTO_INTERNAL_TAG_LINKS_MAX_PER_POST — read-only from this endpoint.               |
| `maxPerTag`               | integer                | yes      | no       | Deployment-wide BLOG_AUTO_INTERNAL_TAG_LINKS_MAX_PER_TAG — read-only from this endpoint.                |
| `minTermLength`           | integer                | yes      | no       | Deployment-wide BLOG_AUTO_INTERNAL_TAG_LINKS_MIN_TERM_LENGTH — read-only from this endpoint.            |
| `linkFirstOccurrenceOnly` | boolean                | yes      | no       | Deployment-wide BLOG_AUTO_INTERNAL_TAG_LINKS_LINK_FIRST_OCCURRENCE_ONLY — read-only from this endpoint. |
| `excludeHeadings`         | boolean                | yes      | no       | Deployment-wide BLOG_AUTO_INTERNAL_TAG_LINKS_EXCLUDE_HEADINGS — read-only from this endpoint.           |
| `updatedAt`               | string (date-time)     | yes      | yes      |                                                                                                         |
...
```json
{
  ...
  "disabledTagIds": ["00000000-0000-0000-0000-000000000000"],
  "maxPerPost": 0,
  "maxPerTag": 0,
  "minTermLength": 0,
...
### Schema: InternalTagLinkMatch

| Field         | Type          | Required | Nullable | Description |
| ------------- | ------------- | -------- | -------- | ----------- |
| `tagId`       | string (uuid) | yes      | no       |             |
| `tagName`     | string        | yes      | no       |             |
| `url`         | string        | yes      | no       |             |
| `matchedText` | string        | yes      | no       |             |
...
```json
{
  "providerKey": "linkedin_organization",
  "providerAccountId": "urn:li:organization:12345678",
  "providerAccountName": "Acme Corp",
  "providerAccountType": "organization",
  "tokenReference": "env:LINKEDIN_ACME_ORG_TOKEN",
  "scopes": [
...
| `id`                  | string (uuid)                                                         | yes      | no       |                                                                                                                                                                                                                                                                                                                                                  |
| `tenantId`            | string (uuid)                                                         | yes      | no       |                                                                                                                                                                                                                                                                                                                                                  |
| `providerKey`         | string                                                                | yes      | no       | Provider-neutral by design — not a fixed enum (see migration 050's header). Concrete values are minted by adapter issues (#644/#645/#646) — e.g. `linkedin_organization` (#645, LinkedIn organization pages; `providerAccountId` is the full `urn:li:organization:{id}`, `providerAccountType` is `organization`) and `telegram_channel` (#646). |
| `providerAccountId`   | string                                                                | yes      | no       |                                                                                                                                                                                                                                                                                                                                                  |
| `providerAccountName` | string                                                                | yes      | no       |                                                                                                                                                                                                                                                                                                                                                  |
| `providerAccountType` | enum(`page`, `profile`, `channel`, `group`, `organization`)           | yes      | no       |                                                                                                                                                                                                                                                                                                                                                  |
...
### Schema: SocialAccountVerifyResult

| Field         | Type               | Required | Nullable | Description                                                                                                                                |
| ------------- | ------------------ | -------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `accountId`   | string (uuid)      | yes      | no       |                                                                                                                                            |
| `providerKey` | string             | yes      | no       |                                                                                                                                            |
| `valid`       | boolean            | yes      | no       |                                                                                                                                            |
| `reason`      | string             | no       | yes      | Sanitized, safe-to-display machine reason code (e.g. `missing_channel_permission`, `channel_not_found`) — never a raw provider error body. |
| `verifiedAt`  | string (date-time) | no       | yes      | The moment this check (or the most recent prior successful check) succeeded — `null` if never successfully verified.                       |
...
| `slug`            | string                                         | no       | no       |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `excerpt`         | string                                         | no       | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `contentJson`     | object                                         | no       | no       | When full-online R2-only news portal mode is active for this tenant (Issue #636), an image gallery block item (mediaType image) must use mediaObjectId referencing a verified/attached same-tenant media object, never a raw url. A video_news block (Issue #639) requires an allowlisted provider (currently only youtube) and a valid videoId (normalized server-side from a bare id or common video URL); its optional thumbnailMediaObjectId, when full-online R2-only mode is active, must likewise reference a verified/attached same-tenant media object. |
...
| `metaDescription`              | string                                | no       | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `canonicalUrl`                 | string                                | no       | yes      |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `termIds`                      | array of string (uuid)                | no       | no       | Replaces the full category/tag assignment set for this post (Issue #539).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `autoInternalTagLinksDisabled` | boolean                               | no       | no       | Manual per-post opt-out of automatic internal tag linking (Issue #641). Defaults false.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `socialPreviewFallbackImageMediaId`        | string (uuid)                                                            | no       | yes      |             |
| `socialPreviewContentImageFallbackEnabled` | boolean                                                                  | no       | no       |             |
...
### Schema: UpdateInternalTagLinkingSettingsInput

Partial update — only fields present are validated/changed. disabledTagIds must reference existing, same-tenant, non-deleted taxonomy_type=tag terms.

| Field             | Type                   | Required | Nullable | Description |
| ----------------- | ---------------------- | -------- | -------- | ----------- |
| `enabled`         | boolean                | no       | no       |             |
| `caseInsensitive` | boolean                | no       | no       |             |
| `disabledTagIds`  | array of string (uuid) | no       | no       |             |
...
- `awcms-mini.blog-content.internal-tag-linking-policy.updated` — This tenant's automatic internal tag linking policy (enabled, caseInsensitive, disabledTagIds — `awcms_mini_blog_internal_tag_link_ settings`) was updated (Issue #641). Documented contract only; producer is the structured JSON logger, invoked from `pages/api/v1/blog/internal-tag-links/settings.ts`'s `PATCH` handler (`blog-content.internal-tag-linking-policy.updated` log line).
...
- `awcms-mini.social-publishing.account.verification-failed` — A connected social account's verification check failed (e.g. the bot lacks channel-administrator permission, or a Meta token has expired/ lost a required scope) (Issue #646). Informational — does NOT itself change `connection_status`/`auto_publish_enabled`. Producer: `social-account-directory.ts`'s `recordSocialAccountVerification`.
- `awcms-mini.social-publishing.account.verified` — A connected social account was verified — the provider adapter confirmed the credential can post to the connected target (Issue #646, the Telegram channel adapter's `verifyCredentials`; also exercised by the Meta adapter's `verifyCredentials`, Issue #644). Producer: `social-account-directory.ts`'s `recordSocialAccountVerification`, called from `POST /api/v1/social-publishing/accounts/{id}/verify`.
