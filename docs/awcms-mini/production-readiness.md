...
| Backup aktif / restore tested                 | **Manual** (SOP + skrip sudah ada di `deploy/backup/{backup,restore}-postgres.sh` sejak Issue 12.2, dikeraskan Issue #691: enkripsi + manifest bertanda tangan + checksum-before-restore + restore drill terjadwal — lihat skill `awcms-mini-production-preflight` dan `deploy/backup/README.md`)                                                                   |
