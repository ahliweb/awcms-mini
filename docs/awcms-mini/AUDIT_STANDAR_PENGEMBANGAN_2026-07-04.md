...
1. Kerjakan Issue 12.1 untuk setup wizard API — sekarang bisa dikerjakan dengan aman karena base sudah SSR (bukan static) dan kontrak response konsisten dengan doc 05/10.
2. Baru lanjut ke tenant, profile, auth, ABAC, sync, UI, reporting, workflow, dan deployment sesuai urutan docs.
3. Item menengah/rendah yang belum ditindak (tidak memblokir Issue 12.1): tipe `SoftDeleteColumns.deletedAt` di `soft-delete.ts` memakai `Date | null` wajib, sedangkan doc 10 menetapkan `string | null` opsional; `.env.example` mewarisi nama provider spesifik-domain (`STARSENDER_ENABLED`, dst.) dari contoh doc 18; README kecil baru di `src/lib/`, `src/modules/_shared/`, `openapi/`, `asyncapi/`, `deploy/`, `fixtures/` belum diindeks di doc 13; 4 changeset yang terkumpul (`foundation-skeleton`, `db-migration-runner`, `api-contract-baseline`, `pin-ci-actions`) belum dikonsumsi — pertimbangkan apakah pantas bump ke `0.1.0` (slot "Foundation" di doc 09) atau tunggu Tenant/Identity/Profile (Epic 2) selesai.
