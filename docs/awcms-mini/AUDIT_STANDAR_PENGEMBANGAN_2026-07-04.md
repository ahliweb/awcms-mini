...
1. Issue 2.1 (tenant/office), 2.2 (central profile), 2.3 (identity/login), dan 2.4 (RBAC/ABAC) selesai — seluruh epic "Tenant, Identity, Profile" (M2) tuntas.
2. Lanjut ke Issue 12.1 (Setup Wizard API, Sprint 3) — sekarang skema tenant/identitas/RBAC/ABAC lengkap tersedia untuk diinisialisasi (tenant, owner identity, office, role Owner generik + assignment, ABAC default). Ini sesuai `docs/awcms-mini/06_github_issues_detail.md` §Koreksi urutan sprint yang menempatkan 12.1 setelah 2.4.
3. Setelah 12.1 selesai, lanjut ke Epic 4 (Sprint 4: logging, pooling, security readiness) sesuai doc 09 §Urutan commit atomic.
4. Target versi: `v0.2.0`-`v0.4.0` = tenant/identity/profile (2.1–2.3, progresif); Issue 2.4 (RBAC/ABAC) menyusul di rilis berikutnya sebagai bagian slot RBAC/ABAC evaluator doc 09 §Versioning. Lihat catatan versi progresif di CHANGELOG untuk detail per rilis.
5. Tidak ada item menengah/rendah terbuka dari audit ini yang memblokir Issue 12.1.
