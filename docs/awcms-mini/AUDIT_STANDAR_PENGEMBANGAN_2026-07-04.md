...
1. Issue 2.1 (tenant/office) dan 2.2 (central profile) selesai. Lanjut Issue 2.3 (Identity Login and Tenant User Membership) — Sprint 2, sudah `status:ready`, satu-satunya sisa Sprint 2.
2. Setelah 2.3 selesai, kerjakan Issue 2.4 (RBAC/ABAC Access Control, Sprint 3) — evaluator default deny/deny overrides allow generik (bukan matriks role retail doc 17).
...
4. Target versi: `v0.2.0` = tenant/identity/profile (2.1–2.3) per doc 09 §Versioning — 2.1 dan 2.2 sudah masuk rilis `0.2.x`, 2.3 menyusul sebelum slot ini dianggap tuntas; `v0.3.0` = RBAC/ABAC (2.4) menyusul 12.1 di Sprint 3 yang sama.
5. Tidak ada item menengah/rendah terbuka dari audit ini yang memblokir Issue 2.3.
