...
1. Issue 2.1 (tenant/office), 2.2 (central profile), dan 2.3 (identity/login) selesai — seluruh Sprint 2 tuntas.
2. Lanjut Issue 2.4 (RBAC/ABAC Access Control, Sprint 3) — evaluator default deny/deny overrides allow generik (bukan matriks role retail doc 17), memakai `awcms_mini_tenant_users`/`awcms_mini_identities` yang sudah ada sebagai anchor assignment.
...
4. Target versi: `v0.2.0` = tenant/identity/profile (2.1–2.3) per doc 09 §Versioning — ketiganya sudah masuk rilis `0.2.x`-`0.4.x` progresif (lihat catatan versi progresif di CHANGELOG); `v0.3.0` = RBAC/ABAC (2.4) menyusul 12.1 di Sprint 3 yang sama.
5. Tidak ada item menengah/rendah terbuka dari audit ini yang memblokir Issue 2.4.
