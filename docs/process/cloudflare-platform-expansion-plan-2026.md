...
This document captures the next planning pass for AWCMS Mini after the earlier alignment and security work landed.
...
- current AWCMS Mini docs, implementation, and scripts
...
- The repo now documents a Cloudflare-delivered frontend plus Hono backend baseline while keeping PostgreSQL remote on a Coolify-managed VPS.
- The remaining deployment work is follow-on integration detail around frontend/backend boundaries and feature-specific Cloudflare services.
...
- keep PostgreSQL behind the Hono backend even when evaluating broader Cloudflare platform usage
...
- do not introduce direct PostgreSQL connectivity from Workers as part of platform expansion work
...
- keep edge runtime code calling backend APIs instead of PostgreSQL directly
