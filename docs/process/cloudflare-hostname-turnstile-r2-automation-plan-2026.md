...
This document captures the next planning pass for AWCMS Mini after the Cloudflare-hosted runtime, Turnstile baseline, R2 storage baseline, and JWT-backed edge API work landed.

This revision focuses on the next operator-facing automation layer for the supported production baseline:

- AWCMS Mini hosted on Cloudflare
...
- current AWCMS Mini requirements, docs, and implementation
- current EmDash-first repository constraints and current EmDash `0.5.0` alignment
...
1. automatically configure `awcms-mini.ahlikoding.com` as the Cloudflare-managed public hostname for the AWCMS Mini public website
2. automatically configure `awcms-mini-admin.ahlikoding.com` as the Cloudflare-managed admin hostname for the AWCMS Mini admin entry path
...
4. automatically provision and bind the Cloudflare R2 bucket `awcms-mini-s3`
...
- treat `awcms-mini.ahlikoding.com` as the canonical `SITE_URL` for public pages and public auth flows
- treat `awcms-mini-admin.ahlikoding.com` as an allowed operator-facing hostname for the EmDash admin entrypoint and associated session flows
...
- use Cloudflare MCP or equivalent Cloudflare-managed automation to ensure both custom domains are attached to the intended Worker in account `5255727b7269584897c8c97ebdd3347f`
...
- `awcms-mini.ahlikoding.com` should serve the public site and public auth flows
- `awcms-mini-admin.ahlikoding.com` should serve the admin entrypoint and operator auth flows
...
- provision at least one Turnstile widget for AWCMS Mini using Cloudflare-managed automation
...
- public login on `awcms-mini.ahlikoding.com`
- password-reset request on `awcms-mini.ahlikoding.com`
- invite activation or similar public onboarding routes on `awcms-mini.ahlikoding.com`
- any admin login or operator-facing unauthenticated flow exposed on `awcms-mini-admin.ahlikoding.com`
...
Mini should formally support automated creation and configuration of the R2 bucket `awcms-mini-s3`.

Recommended direction:

- create `awcms-mini-s3` through Cloudflare-managed automation for account `5255727b7269584897c8c97ebdd3347f`
...
- set `R2_MEDIA_BUCKET_NAME=awcms-mini-s3`
...
- use the Cloudflare MCP during setup and operational automation to:
  ...
  - create the `awcms-mini-s3` bucket if it does not exist
...
- the currently available Cloudflare MCP inventory calls did not return visible resource data, so implementation must include a first validation step to confirm the accessible zone, Worker, and R2 state for account `5255727b7269584897c8c97ebdd3347f`
...
- if host-only cookies are used, are login and admin entry flows clear enough for operators moving between `awcms-mini.ahlikoding.com` and `awcms-mini-admin.ahlikoding.com`?
...
- use Worker custom domains for `awcms-mini.ahlikoding.com` and `awcms-mini-admin.ahlikoding.com` when the Worker is the origin
...
1. validate live Cloudflare account and zone access for account `5255727b7269584897c8c97ebdd3347f`
2. add Worker custom-domain automation for `awcms-mini.ahlikoding.com` and `awcms-mini-admin.ahlikoding.com`
...
5. create and bind the `awcms-mini-s3` R2 bucket
...
Recommended GitHub issue: `#129`

- confirm the accessible Cloudflare zone for `ahlikoding.com` and record the required zone ID
- document or automate Worker custom-domain provisioning for `awcms-mini.ahlikoding.com`
- document or automate Worker custom-domain provisioning for `awcms-mini-admin.ahlikoding.com`
- define the runtime rule for public vs admin hostname handling

### Issue B: Add Hostname-Aware Runtime And Admin Entry Rules

Recommended GitHub issue: `#130`
...
### Issue D: Automate R2 Bucket Provisioning And Binding For `awcms-mini-s3`

Recommended GitHub issue: `#132`

- create the R2 bucket `awcms-mini-s3`
