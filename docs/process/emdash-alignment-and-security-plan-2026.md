...
- The installed `emdash` package version is now `0.9.0`, which matches the current local EmDash core package version inspected in a checked-out copy of `emdash-cms/emdash`.
...
For the intended deployment model, document a Cloudflare-plus-Coolify origin strategy with Cloudflare proxied DNS, origin firewalling, and strict trusted-header validation where applicable.
