...
export async function evaluateSetupStatusResponse(response) {
  const contentType = response.headers.get("content-type");
...
export async function runCloudflareAdminSmokeTest({
  baseUrl = resolveSmokeBaseUrl(),
  fetchImpl = fetch,
} = {}) {
  ...
  return {
    ...
    checks: {
      adminEntry,
      setupShell,
      // Keep setupStatus as a diagnostic seam so setup-shell failures are easier to
      // distinguish from broader runtime or database initialization failures.
      setupStatus,
    },
