...
function checkLinks(file, content) {
  ...
  let m;
  const lineOffsets = [];
  { let idx = 0; for (const ln of content.split("\n")) { lineOffsets.push(idx); idx += ln.length + 1; } }
  const lineOf = (pos) => {
    let lo = 0, hi = lineOffsets.length - 1, ans = 0;
    while (lo <= hi) { const mid = (lo + hi) >> 1; if (lineOffsets[mid] <= pos) { ans = mid; lo = mid + 1; } else hi = mid - 1; }
    return ans + 1;
  };
