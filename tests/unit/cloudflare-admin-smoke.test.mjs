...
test("resolveSmokeBaseUrl prefers an explicit argument over env defaults", async () => {
...
test("evaluateAdminEntryAliasResponse accepts the reviewed same-host redirect", async () => {
  ...
  const result = evaluateAdminEntryAliasResponse(response, new URL("https://awcms-mini.ahlikoding.com"));
  assert.equal(result.ok, true);
});

test("evaluateAdminEntryAliasResponse rejects unexpected redirect targets", async () => {
  const response = new Response(null, {
    status: 302,
    headers: {
...
test("evaluateSetupShellResponse rejects non-html setup-shell responses", async () => {
...
test("evaluateSetupShellResponse rejects known runtime error markers", async () => {
...
test("runCloudflareAdminSmokeTest reports setup-status as a diagnostic seam", async () => {
  const responses = new Map([
    [
      `https://awcms-mini.ahlikoding.com${DEFAULT_ADMIN_ENTRY_ALIAS_PATH}`,
      new Response(null, {
        status: 302,
    ...
    [
      `https://awcms-mini.ahlikoding.com${DEFAULT_SETUP_SHELL_PATH}`,
    ...
    [
      `https://awcms-mini.ahlikoding.com${DEFAULT_SETUP_STATUS_PATH}`,
