...
describe("validateDomainEventPayload (Issue #742)", () => {
  ...
  test("rejects a payload with a secret-shaped value regardless of key name", () => {
    const result = validateDomainEventPayload({
      // JWT-shaped value under an innocuous key name — same obviously-fake
      // signature suffix `tests/audit-log.test.ts` and `tests/unit/error-
      // sanitizer.test.ts` already use (a YouTube id + "_test_signature",
      // never a plausible real signature), to avoid GitGuardian flagging a
      // more realistic-looking fabricated JWT as a genuine leaked secret.
      publicLabel:
