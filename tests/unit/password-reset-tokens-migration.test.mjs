...
import { down, up } from "../../src/db/migrations/028_password_reset_tokens.mjs";
...
test("password_reset_tokens migration stores hashed tokens with expiry and optional issuer linkage", async () => {
  const { db, operations } = createFakeSchemaRecorder();

  await up(db);

  assert.ok(operations.some((entry) => entry[0] === "createTable" && entry[1] === "password_reset_tokens"));
  assert.ok(operations.some((entry) => entry[0] === "addColumn" && entry[1] === "password_reset_tokens" && entry[2] === "token_hash" && entry[3] === "text"));
  assert.ok(operations.some((entry) => entry[0] === "addColumn" && entry[1] === "password_reset_tokens" && entry[2] === "expires_at"));
  assert.ok(operations.some((entry) => entry[0] === "addColumn" && entry[1] === "password_reset_tokens" && entry[2] === "used_at"));
  assert.ok(operations.some((entry) => entry[0] === "addColumn" && entry[1] === "password_reset_tokens" && entry[2] === "issued_by_user_id"));
  assert.ok(operations.some((entry) => entry[0] === "createIndex" && entry[1] === "password_reset_tokens_user_id_index"));
  assert.ok(operations.some((entry) => entry[0] === "createIndex" && entry[1] === "password_reset_tokens_expires_at_index"));
});

test("password_reset_tokens migration down removes indexes and table", async () => {
  const { db, operations } = createFakeSchemaRecorder();
  ...
  assert.deepEqual(
    ...
    [
      ["dropIndex", "password_reset_tokens_expires_at_index"],
      ["dropIndex", "password_reset_tokens_user_id_index"],
