...
suite("tenant module preset application service", () => {
  ...
  test("applying online_website enables its listed modules (all default-enabled already, so no changes), disables the safely-disableable non-listed modules, and skips one blocked by a transitive dependency", async () => {
    ...
    // (Issue #749), document_infrastructure (Issue #751), and
    // integration_hub (Issue #754) all depend on tenant_admin/
    // identity_access/domain_event_runtime, all of which stay enabled or
    // disable in this same pass — nothing depends on any of them, also
    // pure leaves, and all three disable BEFORE domain_event_runtime
