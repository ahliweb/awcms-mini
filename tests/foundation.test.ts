...
describe("module registry", () => {
  test("tenant_admin, profile_identity, identity_access, sync_storage, reporting, logging, workflow, form_drafts, email, module_management, blog_content, tenant_domain, visitor_analytics, news_portal, idn_admin_regions, social_publishing, data_lifecycle, domain_event_runtime, organization_structure, document_infrastructure, data_exchange, integration_hub, reference_data, service_catalog, tenant_entitlement, and tenant_provisioning are registered after Issue 2.1-2.4, 12.1, 6.1-6.3, 9.1, 10.1, 11.1, #484, #493-#498, #511-#513, #537, #558, #617, #632, #655, #643, #745, #742, #749, #750, #751, #752, #754, #870, #871, #872", () => {
    ...
    expect(getModuleByKey("organization_structure")).toMatchObject({
    ...
    // control-plane module: default-disabled, provides service_catalog_read.
    expect(getModuleByKey("service_catalog")).toMatchObject({
      key: "service_catalog",
      status: "active",
      type: "domain",
...
describe("database migration runner helpers", () => {
  ...
  test("real sql/ migrations are discoverable, ordered, and transaction-control-free", async () => {
    ...
    expect(migrations.map((migration) => migration.name)).toEqual([
      ...
      "066_awcms_mini_document_infrastructure_schema.sql",
      "067_awcms_mini_document_infrastructure_permissions.sql",
      "068_awcms_mini_document_infrastructure_confidentiality_permissions.sql",
      "069_awcms_mini_reporting_projections_schema.sql",
      "070_awcms_mini_reporting_projections_permissions.sql",
      "071_awcms_mini_data_exchange_schema.sql",
      "072_awcms_mini_data_exchange_permissions.sql",
