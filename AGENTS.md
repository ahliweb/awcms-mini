...
| Modul tenant_lifecycle — state machine lifecycle tenant, restriksi fail-closed server-derived lintas API/SSR/public/worker, transisi terjadwal idempoten, downgrade non-destruktif, restore ter-rekonsiliasi (Issue #873)                                                                                                                                                                                                                                                  | `awcms-mini-tenant-lifecycle`        |
| Modul subscription_billing — state komersial SaaS (subscription/invoice/credit/dunning), uang EXACT minor-unit, issued invoice immutable, generasi idempoten per period, dunning via kontrak lifecycle #873; BUKAN general ledger/AR-AP/tax (Issue #876)                                                                                                                                                                                                                   | `awcms-mini-subscription-billing`    |
| Modul payment_gateway — gateway pembayaran provider-netral (checkout/session, webhook signed fail-closed anti-replay durable, event ternormalisasi, refund, retry/DLQ, provider health, reconciliation); status TAK PERNAH dari redirect browser; provider di luar transaksi (outbox); secret hanya di process.env; adapter opsional (sandbox di base); CONSUMES billing_document_state, PROVIDES payment_outcome; BUKAN GL/AR-AP/merchant settlement/tax/PCI (Issue #877) | `awcms-mini-payment-gateway`         |
