...
import { restoreClassification } from "../../../../../../modules/document-infrastructure/application/document-classification-directory";

const IDEMPOTENCY_SCOPE = "document_infrastructure_classification_restore";
...
export const POST: APIRoute = async ({ request, cookies, params, locals }) => {
  ...
  return withTenant(sql, tenantId, async (tx) => {
    ...
    const result = await restoreClassification(
      tx,
      tenantId,
    ...
    if (!result.ok) {
      ...
      return fail(
        409,
        "NOT_DEACTIVATED",
        "Classification is not currently deactivated."
      );
    }
