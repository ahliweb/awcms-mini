...
/** Naturally idempotent — no `Idempotency-Key` required (same reasoning as pause.ts). */
export const POST: APIRoute = async ({ request, params, cookies, locals }) => {
  ...
  return withTenant(sql, tenantId, async (tx) => {
    ...
    await recordAuditEvent(tx, {
      ...
      moduleKey: "integration_hub",
      action: "integration_hub.endpoint.resumed",
      resourceType: "integration_endpoint",
      resourceId: id,
      severity: "info",
      message: "Inbound webhook endpoint resumed.",
      correlationId
    });
