 ...
 * more sensitive there does not automatically leak. There are exactly TWO
 * exceptions, both at the bottom of this file:
 * `fetchSocialAccountTokenReferenceForDispatch` — used ONLY by
 * `social-publish-dispatch.ts` (an internal worker, never an HTTP route) —
 * and `fetchSocialAccountTokenReferenceForVerification` (Issue #644) — used
 * ONLY by `social-account-verification.ts`'s manual "verify connection"
 * admin action, which immediately hands the value to
 * `adapter.verifyCredentials()` and discards it; never returned in any HTTP
 * response. The column list is repeated literally at each query site (not
 * factored into a shared fragment) — same convention every other directory
 * module in this repo uses (e.g. `ad-placement-directory.ts`).
 */
...
/**
 * SECOND exception (Issue #644) — see this file's header comment. Used
 * ONLY by `social-account-verification.ts`'s manual "verify connection"
 * admin action. Includes `provider_account_type`/`connection_status` (not
 * present on the dispatch-only accessor above) because the verification
 * orchestrator needs both to decide "unsupported account type" and whether
 * a state transition is meaningful, without a second round-trip.
 */
export async function fetchSocialAccountTokenReferenceForVerification(
  tx: Bun.SQL,
  tenantId: string,
  id: string
): Promise<{
  ...
  const rows = (await tx`
    SELECT provider_key, provider_account_type, connection_status, token_reference, scopes_json
    FROM awcms_mini_social_accounts
    WHERE tenant_id = ${tenantId} AND id = ${id}
  `) as {
    provider_key: string;
...
/** Records a successful "verify connection" check — only touches `last_verified_at` (never rotates `token_reference`/`connection_status`; a successful verification does not need to). */
export async function recordSocialAccountVerificationSuccess(
  tx: Bun.SQL,
  tenantId: string,
  id: string,
  correlationId?: string
  ...
  await recordAuditEvent(tx, {
    tenantId,
    moduleKey: AUDIT_MODULE_KEY,
    action: "social_publishing.account.verified",
    resourceType: AUDIT_RESOURCE_TYPE,
    resourceId: id,
    severity: "info",
    message: `Social account connection verified: ${updated.providerKey}/${updated.providerAccountName}.`,
    correlationId
  });
