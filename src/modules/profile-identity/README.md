...
Skema dasar di `sql/003_awcms_mini_central_profile_management_schema.sql`; ekstensi Issue #748 di `sql/059_awcms_mini_profile_identity_party_lifecycle_schema.sql`. Seluruh tabel tenant-scoped memakai `ENABLE`+`FORCE ROW LEVEL SECURITY` — 7 tabel migration 003 sudah mendapat `FORCE` sejak migration `013_awcms_mini_enforce_rls_least_privilege.sql` (PR #777 review correction — draft awal salah mengklaim migration 059 menutup gap ini; statement `FORCE` yang diulang di 059 adalah no-op aman, sekadar keterbacaan mandiri file itu) — lihat `docs/awcms-mini/04_erd_data_dictionary.md` §RLS standard.
