...
Sampai Issue #179, `evaluateAccess` tidak pernah membaca baris `awcms_mini_abac_policies` — otorisasi hanya RBAC + guard bawaan. Issue ini menghubungkan **kebijakan tersimpan** ke chokepoint `authorizeInTransaction` secara **default-deny** tanpa melemahkan guard yang ada. Keputusan penuh (DSL, precedence, cache) ada di **ADR-0023**; ringkasannya:

- **DSL (`domain/abac-policy.ts`).** `conditions` = AST jsonb terbatas: node `allOf`/`anyOf`/`not` dan leaf `{attr, op, value}` atau `{attr, op, valueAttr}` (attr-ke-attr untuk cek kepemilikan). Attribute dari **allow-list server-side** (`subject.*` dari context terautentikasi — bukan body klien; `resource.*` dari `request.resourceAttributes` yang wajib diisi endpoint dari resource nyata; `action`; `env.*` server-derived, `env.ipTrusted` default `false`). Operator: `eq/ne/in/nin/lt/lte/gt/gte/exists` (perbandingan hanya numeric/date). `dsl_version` mulai 1. Parser/validator **fail-closed**: attribute/operator tak dikenal, tipe nilai salah, atau versi terlalu baru → invalid saat authoring, tak bisa disimpan/diaktifkan.
- **Evaluator (`domain/abac-evaluator.ts`).** Interpreter **murni** atas AST — tanpa `eval`/`new Function`/dynamic import/SQL. `evaluateAccess` memperoleh param opsional ke-5 `abac?: { policies, env }`; bila absen/kosong → ABAC no-op (semua call site lama ≤4 argumen tak terpengaruh).
- **Precedence (fail-closed).** Setelah guard bawaan (tenant isolation, self-approval, force-decision, business-scope) dan filter applicability (`module_key`/`activity_code`/`action`/`resource_type`, nullable = wildcard): (1) **DENY eksplisit menang** — kebijakan `deny` yang kondisinya terpenuhi, kebijakan aktif invalid, atau error evaluasi apa pun → DENY, **sebelum** cek RBAC; (2) **permission RBAC tetap wajib** — kebijakan `allow` tak pernah menciptakan permission; (3) kebijakan `allow` sebagai **constraint** — bila ada yang applicable, minimal satu harus terpenuhi, jika tidak → DENY (`abac_allow_unsatisfied`); tanpa kebijakan applicable → ABAC no-op. Atribut sah-tapi-absen membuat leaf `false` (deterministik, bukan error).
- **Cache (`application/policy-cache.ts`).** Kebijakan aktif dikompilasi sekali per tenant, di-cache in-process **tenant-keyed**, di-invalidasi **deterministik** oleh setiap create/update/enable/disable (`invalidatePolicyCache` menaikkan versi per-tenant + hapus entry; endpoint memanggilnya **setelah commit** agar tak men-cache snapshot pra-commit). Load selalu di `withTenant` (RLS + peran `awcms_mini_app`) → tak pernah lintas tenant, tanpa restart. Batasan: invalidasi per-proses (multi-instance butuh LISTEN/NOTIFY/TTL).
- **Decision log.** `recordDecisionLog` mencatat `decision`, `reason`, `matched_policy` (kode), dan `matched_policy_version` (`sql/083`) — tanpa PII/identifier sensitif mentah.
- **Admin API.** `GET/POST /api/v1/access/policies`, `GET/PUT /api/v1/access/policies/{id}`, `POST /api/v1/access/policies/{id}/{enable,disable}` (guard `identity_access.abac_policies.{read,configure}`, audited, hanya DSL valid yang tersimpan), dan `POST /api/v1/access/policies/simulate` — simulasi **read-only** (guard `.analyze`) atas subject/resource/action hipotetis → decision + trace per-kebijakan (hanya boolean struktural, tanpa nilai atribut) → tidak memutasi data domain, hanya menulis audit event. Permission di-seed `sql/084`. Endpoint `POST /api/v1/access/evaluate` kini juga mengonsumsi kebijakan aktif sehingga mencerminkan keputusan nyata.
- **Contoh (bukan base).** Base **tidak** menyertakan kebijakan domain. Lima contoh ERP (kepemilikan, batas nilai, immutability posted, gate environment, lifecycle draft) ada di `fixtures/abac-example-policies.json` untuk di-author aplikasi turunan lewat API.
...
- Helper baru `src/lib/auth/ssr-session.ts` (`resolveSsrContext` → `loadSsrSessionData`) membaca kedua cookie dan menyusun konteks yang setara dengan `POST /access/evaluate`, hanya sumber tenant/token-nya cookie, bukan header. Mengembalikan `null` (tanpa melempar error) bila cookie tidak ada atau sesi tidak valid; layout SSR meng-redirect ke `/login`.
  - **Gate module-enabled (Issue #841).** `context.permissions` yang dibangun di sini **membuang** setiap permission key yang modulnya `awcms_mini_tenant_modules.enabled = false`. Jalur API menolak `403 MODULE_DISABLED` (`resolveModuleEnabled` di `authorizeInTransaction`) SEBELUM RBAC, tetapi ke-54 halaman admin SSR yang memuat data hanya menggerbang lewat `context.permissions.has(permissionKey(...))` — jadi tanpa filter ini, men-disable modul membuat route-nya 403 sementara halaman admin-nya tetap merender data tenant. Filter ditaruh di helper bersama ini, bukan di 54 call site, dan `fetchGrantedPermissionKeys` (dipakai jalur API) sengaja **tidak** diubah (beberapa endpoint mengandalkannya untuk TIDAK memfilter modul disabled). Role tidak difilter — identitas subject tetap; hanya kapabilitas modul disabled yang hilang dari set SSR. Diuji di `tests/integration/ssr-session-module-gate.integration.test.ts` (assert kedua sisi + mekanismenya: key tetap granted oleh `fetchGrantedPermissionKeys`, tapi hilang dari set SSR).
  - **Satu query gabungan (Issue #835 §7).** Lookup sesi tetap query tersendiri (menghasilkan `identity_id`), lalu SATU query menyelesaikan tenant-user + `default_locale` + roles + permission (tergerbang modul) — 2 round-trip DB, bukan 5 serial seperti sebelumnya. `LEFT JOIN` mempertahankan role tanpa permission dan tenant tanpa baris `tenants`; query `roles` sengaja TIDAK digabung ke query permission karena itu akan menghilangkan role yang tak punya permission.
