...
- Endpoint `GET /api/v1/logs/observability/dependency-health` (Issue #698, epic #679 platform-hardening) — bearer-session, guard `logging.observability.read`. The authorized counterpart to the public `/api/v1/health` (liveness) and `/api/v1/database/pool/health` endpoints: distinguishes `localDependencies` (the database this instance always depends on — `healthy`/`degraded`/`unhealthy` derived from the DB circuit breaker state plus per-work-class saturation) from `optionalProviders` (email, object storage, Cloudflare DNS, SSO/OIDC, ... — each reported only as a bounded, code-defined provider "family" and circuit-breaker state, never a raw registry key that could embed a tenant id). Does not re-ping the database with its own `SELECT 1` — `withTenant` resolving the caller's session on this connection is itself proof of reachability.
...
- **Registry SLO/alert kontribusi-modul** (Issue #930, epic #868) — `domain/slo-registry.ts`. Modul mana pun mendeklarasikan `ServiceLevelObjectiveDescriptor` di `module.ts`-nya sendiri (`serviceLevelObjectives`); modul ini hanya MENGAGREGASI dan MEMVALIDASI, tidak pernah mengarang objective dan tidak pernah menyentuh schema modul lain — pola "modul mendeklarasikan, engine pusat membaca `listModules()`" yang sama dengan `dataLifecycle`/`reportingProjections`.

  Dua aturan yang benar-benar menahan beban keamanan, keduanya di-gate `bun run slo:registry:check` (bagian `bun run check`):
  1. `metricName` WAJIB sudah terdaftar di `METRIC_DEFINITIONS` (`src/lib/observability/metrics-port.ts`). Objective yang diukur terhadap metrik yang tidak pernah di-emit itu **diam permanen** — terlihat seperti coverage di dashboard, tapi tidak pernah memanggil siapa pun.
  2. `dimension` WAJIB salah satu dari `allowedLabelKeys` metrik itu sendiri. Inilah yang membuat kriteria "alert memakai dimensi berkardinalitas rendah" benar **secara konstruksi**: sebuah objective tidak bisa memperkenalkan label baru, jadi tidak bisa memasukkan tenant id/resource id sebagai dimensi alert.
