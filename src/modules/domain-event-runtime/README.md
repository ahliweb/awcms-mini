...
2. **Static consumer registry** — `infrastructure/consumer-registry.ts`'s `DOMAIN_EVENT_CONSUMERS` is a plain, reviewed-source-code array (not a runtime registration call). Each entry declares which event types/versions it wants and a `handler`. Fan-out is decided **at publish time** from this registry, not at dispatch time.
...
Adding a real consumer: add an entry to `infrastructure/consumer-registry.ts`'s `DOMAIN_EVENT_CONSUMERS` array (reviewed source), matching event type(s)/version(s) already present in `domain/event-type-registry.ts`. Adding a real producer: call `appendDomainEvent` inside your own module's transaction, with an event type/version first added to `DOMAIN_EVENT_TYPE_REGISTRY` and a matching AsyncAPI channel.
