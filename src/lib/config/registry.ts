...
export const CONFIG_REGISTRY: readonly ConfigVarEntry[] = [
  ...
  // ---------------------------------------------------------------------
  {
    ...
    description:
      "Shared HMAC key for three keyed pseudonyms/commitments, each domain-separated so their outputs never correlate: (1) the audit-log client-IP pseudonym (`ipHash`) written by the auth routes — src/lib/security/client-fingerprint.ts; (2) the `usage_metering` unique-dimension pseudonym — src/modules/usage-metering/application/unique-dimension-pseudonym.ts; (3) the `tenant_provisioning` owner-secret commitment folded into `inputs_hash` — src/modules/tenant-provisioning/application/owner-secret-commitment.ts. Despite its name it does NOT sign session tokens: sessions are opaque random tokens (`awcms_mini_sessions.token_hash`) and src/lib/auth/jwt-verify.ts verifies provider ID tokens with RS256 against published JWKS, never with this secret. Must be a high-entropy value and must NOT be left at the documented placeholder — all three consumers refuse to start rather than degrade to an unkeyed digest, because each keyed value is persisted and would otherwise be brute-forceable by anyone with database read access (the 2^32 IPv4 space is reversible in seconds; a provisioning `inputs_hash` would become an offline cracking oracle for the tenant owner's initial password). Rotating it breaks `ipHash` correlation across the rotation boundary (audit rows before and after no longer group by source), changes every subsequent unique-dimension pseudonym, and makes an in-flight provisioning `Idempotency-Key` replay return a clean 409 instead of the original response.",
